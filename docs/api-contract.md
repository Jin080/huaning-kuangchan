...
文件上传口径：管理员正式入口为 Bearer JWT；仅在上述开发头模式显式开启时，开发期管理员可带 `x-user-id`、`x-user-role: ADMIN` 调用 `POST /api/files/upload`。请求为 `multipart/form-data`，文件字段名固定为 `file`，表单字段 `category` 当前支持 `LOT_IMAGE` 与 `INSPECTION_REPORT`，可选 `lotId`、`enterpriseId`。图片支持 `image/jpeg`、`image/png`，检测报告支持 `application/pdf`，单文件上限 10MB。响应示例：
