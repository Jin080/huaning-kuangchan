...
| T37 | IN_PROGRESS | 前端视觉 | 门户与详情页 Stitch 全面复刻 | T36 | T37A 执行会话 / 总控复核 | T37A 门户通用组件与首页已完成；T37B/T37C 仍待后续分批 |
...
- T37B 可启动：门户列表与普通详情页 Stitch 复刻，继续基于 `docs/qa/stitch-full-replication-plan.md`，不要重复改 T37A 已完成的门户头部/首页/页脚。
- T32/T33 属于上线必做能力；T37/T38 属于视觉复刻链路，建议不要和认证、上传能力混做同一会话。

## 当前必须串行任务

- T37B/T37C 应等待 T37A 总控提交后再继续，避免多个会话同时改 `PortalPages.tsx`、`index.css` 造成冲突。
