...
| T37 | IN_PROGRESS | 前端视觉 | 门户与详情页 Stitch 全面复刻 | T36 | T37A/T37B 执行会话 / 总控复核 | T37A 门户通用组件与首页、T37B 门户列表与普通详情已完成；T37C 仍待后续分批 |
...
| T38 | TODO | 前端视觉 | 后台与企业中心 Stitch 全面复刻 | T36 | 待排期 | 基于 T36 计划，复刻后台管理与企业中心页面视觉，同时保持真实 API 和现有业务交互 |

## 当前可并行任务

- T37C 可启动：门户登录、企业入驻、资源页决策；如新增 `/resources`、`/resources/detail`，需先确认路由范围，不与 T33 登录/JWT 或 T32 上传能力混做。
- T32/T33 属于上线必做能力；T37/T38 属于视觉复刻链路，建议不要和认证、上传能力混做同一会话。

## 当前必须串行任务

- T37C 应等待 T37B 总控提交后再继续，避免多个会话同时改 `PortalPages.tsx`、`index.css` 造成冲突。
- T32/T33 上线必做能力仍需排期，尤其 T33 完成后必须重跑主流程、敏感附件权限和企业数据隔离验收。
